...
def forward(dialog_encoder, batch, params, output_nsp_scores=False, output_lm_scores=False, sample_size=None,
            evaluation=False):
    tokens = batch['tokens']
    ...
    if output_nsp_scores and output_lm_scores:
        lm_nsp_loss, lm_loss, nsp_loss, nsp_scores, lm_scores = dialog_encoder(tokens, sep_indices=sep_indices,
                                                                               sep_len=sep_len \
        ...
        if ((iter_id % (num_iter_per_epoch * (8 // params['sequences_per_image']))) == 0) and iter_id > 0:
            ...
            for metric_name, metric_value in all_metrics.items():
                ...
                if 'round' in metric_name:
                    viz.linePlot(iter_id, metric_value,
